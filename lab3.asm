...
;	Fall 2014
;	MSP430G2553
;	Draws an 8x8 pixel box. Controlled by buttons.
;-------------------------------------------------------------------------------
...
while1:
	...
	jz Left
	bit.b	#16, &P2IN					; bit 4 of P1IN set?
	jz	Down
	bit.b	#32, &P2IN					; bit 5 of P1IN set?
	jz Up
...
while2: ;this is used to jmp from button pressed

	push R10 ;R10 is used to count to 8
	clr R10 ;ensures it is 0
DrawBox: ;jumps back here to draw box
	inc R10 ; incremented to count to 8
	mov		#NOKIA_DATA, R12			; For testing just draw an 8 pixel high
	mov		#0xFF, R13					; beam with a 2 pixel hole in the center
	call	#writeNokiaByte


	and.w #0x7, R10 ;mod to see if is zero or not.
	jnz DrawBox
...
Right:
	add #0x08, R11
Right2:	bit.b #2, &P2IN					; bit 1 of P1IN clear?
	jz	Right2				; Yes, branch back and wait
	jmp while2
Left:
	sub #0x08, R11
Left2:	bit.b #4, &P2IN					; bit 2 of P1IN clear?
	jz	Left2				; Yes, branch back and wait
	jmp while2
Up:
	dec		R10							; since rows are 8 times bigger than columns
	and.w	#0x07, R10					; wrap over the row mod 8
Up2:	bit.b #32, &P2IN					; bit 4 of P1IN clear?
	jz	Up2				; Yes, branch back and wait
	jmp while2
Down:
	inc		R10							; since rows are 8 times bigger than columns
	and.w	#0x07, R10					; wrap over the row mod 8
Down2:	bit.b #16, &P2IN					; bit 5 of P1IN clear?
	jz	Down2				; Yes, branch back and wait
	jmp while2
